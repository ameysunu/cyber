...
{ 
	...
	std::string ipAddress; 
	 ...
	 char ipAddress1[16];  	
	 strcpy(ipAddress1, ipAddress.c_str());  	
	 SOCKADDR_IN addr;  
...
{ 
	...
	while (true) 
	 	{ 
		  ...
		  strcpy(msg1, userinput.c_str());  	 	
		  for (i = 0; msg1[i] != NULL; i++) 
 	 	 	m1[i] = msg1[i];  	 
			ce(); 
	 	 	encrypt1(); 
		...
		send(Connection, userinput.c_str(), bufferlength, NULL);  
 	 	 	 	 ...
 	 	 	 	 //Code for disconneting Client  	 	
					 if (bufferlength < 11) 
	 	 	{ 
   strcpy(Keyword, userinput.c_str());    
   if (strcmp(terminator, Keyword) == 0) 
	 	 	 	{ 
	 	 	 	 	break; 
	 	 	 	} 
	 	 	} 
...
int prime(long int pr) {  	int i;  	j = sqrt(pr);  	for (i = 2; i <= j; i++) 
{  	 	if (pr%i == 0) 
	 	 	 	return 0; 
	 	} 
	 	return 1; 
